...
INPUT_FILE = THIS_DIR / "low_beta_stratified_linear.input"
...
def main() -> None:
    ...
    kx_index = int(base_k_indices[1])
    ky_index = int(base_k_indices[2])
    kz_max = nz // 2
    ...
    # For this chosen family of modes the low-beta DR is
    #
    #   gamma^2 = N2 * ky^2 / k_perp^2 - vA^2 * k_parallel^2
    #
    # and for this input file (kx, ky) = (0, 1) with vA = 1, this reduces to
    #
    #   gamma^2 = N2 - k_parallel^2.
    #
    # Here we only plot the real growth-rate branch, so the stable region is
    # clipped to gamma = 0 on the curve.
    theory_kpar = np.linspace(0.0, 2.0 * np.pi * kz_max / lz, 400)
    theory_gamma_sq = N2 * ky**2 / kperp**2 - (vA * theory_kpar) ** 2
    theory_gamma = np.sqrt(np.clip(theory_gamma_sq, 0.0, None))

    fig, ax = plt.subplots(figsize=(7.0, 4.8), constrained_layout=True)
    ax.plot(theory_kpar, theory_gamma, lw=2.0, label=r"Linear DR: $\gamma = \sqrt{\max(\gamma^2, 0)}$")
    ...
    ax.set_title("Low-beta stratified linear growth scan")
