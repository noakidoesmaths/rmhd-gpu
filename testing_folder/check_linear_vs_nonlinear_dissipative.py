...
def evolve(linear, diss=3e-3, tmax=80.0, dt=5e-3):
    # stable dispersion relation: g=0.5, K_rho0=1.0  (K_rho >= g/cs^2 = 0.5)
    ...
    try:
        ...
        n=int(round(tmax/dt)); out=[]; Imean=[]; Iacc=0.0
        for i in range(n+1):
            if i%(n//8)==0:
...
for lab,lin in [("LINEAR    + dissipation",True),("NONLINEAR + dissipation",False)]:
    out=evolve(lin)
    print(f"\n=== {lab} (stable DR: g=0.5,K_rho0=1.0) ===")
